...
ParallelBufferPoolManager::ParallelBufferPoolManager(size_t num_instances, size_t pool_size, DiskManager *disk_manager,
    ...
    : pool_size_(pool_size), num_instances_(num_instances), begin_(0) {
  // Allocate and create individual BufferPoolManagerInstances
  for (size_t i = 0; i < num_instances; ++i) {
    bpmis_.push_back(new BufferPoolManagerInstance(pool_size, num_instances, i, disk_manager, log_manager));
  }
}
...
auto ParallelBufferPoolManager::NewPgImp(page_id_t *page_id) -> Page * {
  ...
  // is called
  size_t start = begin_;
  size_t end = start + num_instances_;
  for (size_t i = start; i < end; ++i) {
    Page *p = bpmis_[i % num_instances_]->NewPage(page_id);
