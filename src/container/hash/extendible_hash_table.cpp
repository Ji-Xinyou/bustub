...
HASH_TABLE_TYPE::ExtendibleHashTable(const std::string &name, BufferPoolManager *buffer_pool_manager,
  ...
  //  implement me!
  // fetch dir page
  Page *_dir_pg = buffer_pool_manager->NewPage(&directory_page_id_);
  HashTableDirectoryPage *dir_pg = reinterpret_cast<HashTableDirectoryPage *>(_dir_pg->GetData());
...
auto HASH_TABLE_TYPE::SplitInsert(Transaction *transaction, const KeyType &key, const ValueType &value) -> bool {
  ...
  if (dir_page->GetLocalDepth(bkt1_idx) == dir_page->GetGlobalDepth()) {
    global_changed = true;
  ...
  if (global_changed) {
    uint32_t old_sz = 1 << (dir_page->GetGlobalDepth() - 1);
    uint32_t new_sz = 1 << (dir_page->GetGlobalDepth());
    for (uint32_t i = old_sz; i < new_sz; ++i) {
      if (i == bkt2_idx) {
	  continue;
      }
      uint32_t bucket_idx = i - ((1 << GetGlobalDepth()) >> 1);
  ...
  for (size_t i = shared_bits; i < dir_page->Size(); i += (1 << ld)) {
	...
	dir_page->SetBucketPageId(i, bkt1_page_id);
	dir_page->SetLocalDepth(i, dir_page->GetLocalDepth(bkt1_idx));
      }
  }
...
bool HASH_TABLE_TYPE::RealMerge(Transaction *transaction, HashTableDirectoryPage *dir_page, uint32_t bucket_idx) {
  ...
  for (size_t i = 0; i < dir_page->Size(); ++i) {
    if (dir_page->GetBucketPageId(i) == bucket_page_id) {
      ...
      dir_page->SetLocalDepth(i, dir_page->GetLocalDepth(split_idx));
    }
  }
